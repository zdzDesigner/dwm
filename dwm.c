...
void focusstack(const Arg *arg)
{
    ...
    if (i < 0 || !selmon->sel || selmon->sel->isfullscreen) return;
    ...
    updatenumlockmask();
    {
        ...
        for (i = 0; i < LENGTH(keys); i++)
            if ((code = XKeysymToKeycode(dpy, keys[i].keysym)))
                for (j = 0; j < LENGTH(modifiers); j++) XGrabKey(dpy, code, keys[i].mod | modifiers[j], root, True, GrabModeAsync, GrabModeAsync);
    }
}
...
void manage(Window w, XWindowAttributes *wa)
{
    ...
    setclientstate(c, NormalState);
    if (selmon->sel && selmon->sel->isfullscreen && !c->isfloating) setfullscreen(selmon->sel, 0);
...
void monocle(Monitor *m)
{
    ...
    for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) resize(c, m->wx + ov, m->wy + oh, m->ww - 2 * c->bw - 2 * ov, m->wh - 2 * c->bw - 2 * oh, 0);
}
...
void load_xresources(void)
{
    ...
    for (p = resources; p < resources + LENGTH(resources); p++) resource_load(db, p->name, p->type, p->dst);
